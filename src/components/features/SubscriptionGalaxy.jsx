...
export const SubscriptionGalaxy = ({ subscriptions = [], totalSpend = 0, currency = 'INR' }) => {
  ...
  // Inner: Weekly or under 300 INR
  // Middle: Monthly or under 1500 INR
  // Outer: Yearly or over 1500 INR
  const orbits = {
    inner: [],
    middle: [],
    outer: []
  ...
  activeSubs.forEach(sub => {
    const costInINR = sub.currency === 'USD' ? sub.cost * 83 : sub.cost; // rough normalization for orbit classification
  ...
  const renderPlanet = (sub, index, totalCount, radius) => {
    ...
    // Size of planet based on cost (capped between 28px and 52px)
    ...
    return (
      ...
      >
        {/* Anti-rotation wrapper to keep logos upright */}
        <div className="orbit-counter-1">
          ...
          >
            ...
            {/* Planet Body */}
            <div 
              className="w-full h-full rounded-full border-2 overflow-hidden flex items-center justify-center p-1.5 shadow-lg shadow-black/50 transition-all duration-300"
              style={{ 
            ...
            <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-3 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-50 w-36">
              <div className="glass-premium p-2.5 rounded-xl border border-white/10 text-center shadow-2xl">
                <p className="text-white font-bold text-xs font-display truncate">{sub.name}</p>
                <p className="text-brand-cyan text-xs font-semibold mt-0.5">
              ...
              <div className="w-2 h-2 bg-[#0b0b1a] border-r border-b border-white/10 rotate-45 absolute top-full left-1/2 -translate-x-1/2 -translate-y-1" />
            </div>
          </motion.div>
        </div>
      </div>
  ...
  return (
    <div className="relative w-full aspect-square max-w-[460px] mx-auto flex items-center justify-center bg-mesh rounded-full border border-white/5 p-4 overflow-hidden orbit-pause">
      
      {/* Central Star - Total Spending */}
      <motion.div 
        className="z-10 w-36 h-36 rounded-full glass-premium border border-brand-purple/30 flex flex-col items-center justify-center text-center shadow-[0_0_40px_rgba(139,92,246,0.15)] select-none cursor-pointer"
      ...
      >
        ...
        <span className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold">Total Monthly</span>
      ...
      {/* Orbit Ring 1 - Inner (Radius 90px) */}
      ...
      {/* Orbit Ring 2 - Middle (Radius 145px) */}
      <div className="absolute w-[290px] h-[290px] rounded-full border border-dashed border-white/5 pointer-events-none" />
      ...
      {/* Orbit Ring 3 - Outer (Radius 200px) */}
      <div className="absolute w-[400px] h-[400px] rounded-full border border-dashed border-white/5 pointer-events-none" />
      ...
      <div className="absolute inset-0 pointer-events-none opacity-20">
        <div className="absolute top-[15%] left-[20%] w-1 h-1 bg-white rounded-full animate-ping" />
        <div className="absolute top-[80%] left-[10%] w-1.5 h-1.5 bg-brand-cyan rounded-full animate-pulse" />
        <div className="absolute top-[30%] left-[85%] w-1 h-1 bg-brand-purple rounded-full animate-pulse" />
        <div className="absolute top-[75%] left-[70%] w-1 h-1 bg-white rounded-full animate-ping" />
      </div>
    </div>
  );
};
